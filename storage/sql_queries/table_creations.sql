...
create table Roles (
    ...
    constraint chk_rol check(rol regexp '^[a-zA-ZáéíóúÁÉÍÓÚñÑ]+( [a-zA-ZáéíóúÁÉÍÓÚñÑ]+)*$')
) engine = InnoDB;
...
create table Personas (
    ...
    constraint fk_personas_rol foreign key(rol_id) references Roles(id) on delete restrict on update cascade,
    constraint chk_nombre check(char_length(nombre) between 3 and 30 and nombre regexp '^[a-zA-ZáéíóúÁÉÍÓÚñÑ]{3,}( [a-zA-ZáéíóúÁÉÍÓÚñÑ]{2,})*$'),
	...
	constraint chk_fechaNacimiento check(timestampdiff(year, fechaNacimiento, fechaCreacion) >= 18)
) engine = InnoDB;
...
create table PersonasProcedenDe (
    ...
    constraint fk_personasprocedende_personas
        ...
        on delete cascade
        on update cascade,
	constraint fk_personasprocedende_organizaciones
...
create table Tickets (
    ...
    constraint fk_solicitante_tickets_personas
		foreign key(solicitante_id)
        references Personas(id)
        on delete cascade
        on update cascade,
	constraint fk_tickets_organizacion
		foreign key(organizacion_id)
        references Organizaciones(id)
        on delete cascade
        on update cascade,
...
create table TicketsAprobados (
    ...
    constraint fk_solicitante_ticketsaprobados_personas
        ...
        on delete cascade
        on update cascade,
	constraint fk_ticketsaprobados_organizacion
		foreign key(organizacion_id)
        references Organizaciones(id)
        on delete cascade
        on update cascade,
